/**
 * Встроенный просмотр таблиц (xlsx / xls / csv) без внешней программы.
 * На iPhone Safari/PWA нет встроенного Excel — «Открыть» показывает таблицу в модалке.
 */

import { ensureXlsx } from "./lazy-cdn.js";

const SPREADSHEET_EXT = /\.(xlsx?|csv)$/i;
const SPREADSHEET_MIME = /spreadsheet|excel|csv/i;

/** Максимум строк на лист (включая заголовок), чтобы не подвесить телефон. */
const MAX_ROWS = 800;
/** Максимум столбцов. */
const MAX_COLS = 80;
...
function renderSheet(sheetIndex) {
  ...
  if (statusEl) {
    ...
    if (totalRows > MAX_ROWS) {
      parts.push(`Показаны первые ${MAX_ROWS} из ${totalRows} строк`);
    }
    if ((rows[0]?.length || 0) > MAX_COLS) {
      parts.push(`показаны первые ${MAX_COLS} столбцов`);
    }
    if (parts.length) {
...
/**
 * Загружает файл по URL и показывает таблицу во встроенной модалке.
