...
Чтобы при открытии раздела «Баланс» сохранялся снимок строки «Сейчас», а в «Статистика баланса» можно было смотреть историю, выполни SQL из файла **`supabase_balance_view_logs.sql`** в **Supabase → SQL Editor**. Без этого шага запись просмотров не сохранится (раздел «Баланс» при этом продолжит работать).
