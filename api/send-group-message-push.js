/**
 * Webhook: новая строка в group_messages → push всем участникам группы
 * (кроме отправителя) с активной подпиской.
 *
 * Env (Vercel): те же, что у send-message-push.js
...
function extractMessageRecord(body) {
  ...
  if (body?.id != null && body?.chat_id != null) return body;
  return null;
}
...
async function deleteSubscription(id) {
  try {
    ...
    console.warn("send-group-message-push: delete stale subscription failed:", e.message);
  }
}
...
module.exports = async (req, res) => {
  ...
  if (!record?.id || !record.chat_id) {
    return res.status(400).json({ message: "Missing group message record" });
  }

  webpush.setVapidDetails(VAPID_SUBJECT, VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY);

  try {
    const chatId = encodeURIComponent(record.chat_id);
    const chats = await supabaseRest(
      `group_chats?id=eq.${chatId}&select=id,name,member_ids`,
    );
    const chat = Array.isArray(chats) ? chats[0] : null;
    if (!chat) {
      return res.status(200).json({ sent: 0, reason: "chat_not_found" });
    }

    const senderId = record.sender_id ? String(record.sender_id) : "";
    ...
    console.error("send-group-message-push:", e);
